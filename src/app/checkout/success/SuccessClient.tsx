...
function isNotFoundYet(x: StatusResponse): x is NotFoundYet {
  return "ok" in x && x.ok === false && "found" in x && x.found === false;
}
function isApiError(x: StatusResponse): x is ApiError {
  return "error" in x;
}
...
export default function SuccessClient({ sessionId }: { sessionId: string | null }) {
  ...
  const reservationId = data && isOkFound(data) ? data.reservationId : null;
