...
export default function CancelPageClient() {
  ...
  const [state, setState] = useState<UiState>(
    checkoutSessionId ? "releasing" : "idle",
  );
    ...
    <main className="max-w-xl p-6">
      ...
      {checkoutSessionId ? (
        <div className="mt-4 space-y-2 text-sm text-gray-700">
          {state === "releasing" && (
            <p>
              Estamos liberando tu retencion para devolver las plazas al
              instante...
            </p>
          )}
          ...
          {state === "released" && (
            <p className="text-green-700">
              Tu retencion se ha liberado correctamente.
            </p>
          )}

          {state === "already_released" && (
            <p>
              La retencion ya no estaba activa cuando llegaste a esta pagina.
            </p>
          )}
        ...
        <p className="mt-4 text-sm text-gray-600">
          Falta `session_id` en la URL, asi que no hemos podido localizar la
          reserva.
        </p>
      )}
    </main>
